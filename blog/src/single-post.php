            ...
            function showAllComments() 
            {
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        var row = '';
                        comments.forEach(i => {
                            row += '<div class="single-post__comment__item__text">' +
                                '<h5>' + i['userfullname'] + '</h5>' +
                                '<span>' + i['comment_date'] + '</span>' +
                                '<p id="commentLabel">' + i['post_comment'] + 
                                '</p>' +
                                '<ul>' +
                                '<li><a id="' + i['comment_id'] + '" data-bs-toggle="modal" data-bs-target="#editCommentModalBtn" class="editcommentBtn" ><i class="fa fa-edit"></i></a></li>' +
                                '<li><a id="' + i['comment_id'] + '" class="deletecommentBtn"><i class="fa fa-trash"></i></a></li>' +
                                '</ul>' +
                                '</div>';
                        });

                        $("#commentsDiv").html(row);
                    }
                });
